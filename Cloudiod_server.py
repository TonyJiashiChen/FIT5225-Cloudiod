...
def do_prediction(image,net,LABELS):
    ...
    if len(idxs) > 0:
        ...
        for i in idxs.flatten():
            res_dict['objects'].append("detected item:{}, accuracy:{}, X:{}, Y:{}, width:{}, height:{}".format(LABELS[classIDs[i]],
                                                                                             confidences[i],
                                                                                             boxes[i][0],
                                                                                             boxes[i][1],
                                                                                             boxes[i][2],
...
def process():
    ...
    res_dict = {}
    res_dict['id'] = json_data['id']
    res_dict['objects'] = []
    detect_dict = do_prediction(image, nets, Lables)
    detect_array = detect_dict['objects']
    for i in detect_array:
        res_dict['objects'].append(i)
    return res_dict
